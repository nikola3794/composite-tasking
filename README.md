...
For the proper evaluation of edge detecetion, which is reported in the CompositeTasking paper, the [seism repository](https://github.com/jponttuset/seism) needs to be used (evaluation code is written in MATLAB and it is required to have it installed). In order to do so, edge predictions need to be saved as .png images with the same size as the input image and provided to the seism evaluation protocol along with the labels contained in the dataset. The best place to save the predictions as images in the code is in the training_step() method of `/root/src/systems/system.py`. If there is trouble with running the seism repository, take a look at [this repository](https://github.com/nikola3794/edge-evaluation-PASCAL-MT-tmp), which is a quick and temporary solution with instructions on how to call the seism repository.
