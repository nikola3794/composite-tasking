...
class CompositeTaskingNetV0(nn.Module):
    ...
    def __init__(self, cfg, task_z_code_dict):
        ...
        self.fcs = Intermediate_Generator(n_fc=self.cfg["n_fc_z_map"], input_dim=self.z_code_dim, output_dim=self.cfg["latent_w_dim"], custom_w_init=self.cfg["fc_z_w_custom_w_init"])

        # Create the decoder
        self._create_decoder()
    
    def _normalize_z_l2_dist(self):
        z_i = lambda i: self.task_z_code_dict[i]
        z_i_unit = lambda i: z_i(i) / z_i(i).norm(p=2, dim=0, keepdim=False)
        dist_i_j = lambda i, j: torch.dist(z_i_unit(i), z_i_unit(j), 2)
    ...
    def _check_cfg(self):
        ...
        assert "latent_w_spat_interpolation" in self.cfg
        assert "backbone_arch" in self.cfg
        assert "backbone_pre_trained" in self.cfg
        assert "n_ch_before_output" in self.cfg
        assert "fc_z_w_custom_w_init" in self.cfg
        assert "spade_cond_fn" in self.cfg 

        assert self.cfg["latent_w_spat_interpolation"] in ["bilinear_seq", "bilinear_from_orig", "nearest_seq", "nearest_from_orig"]

    def _create_encoder(self):
        # Load the encoder backbone
        if self.cfg["backbone_arch"] == "resnet18":
            base_model = torchvision.models.resnet.resnet18(pretrained=self.cfg["backbone_pre_trained"], progress=False)
        elif self.cfg["backbone_arch"] == "resnet34":
            base_model = torchvision.models.resnet.resnet34(pretrained=self.cfg["backbone_pre_trained"], progress=False)
        elif self.cfg["backbone_arch"] == "resnet50":
            base_model = torchvision.models.resnet.resnet50(pretrained=self.cfg["backbone_pre_trained"], progress=False)
        elif self.cfg["backbone_arch"] == "resnet101":
            base_model = torchvision.models.resnet.resnet101(pretrained=self.cfg["backbone_pre_trained"], progress=False)
        else:
    ...
    def _create_decoder(self):
        ...
        if self.cfg["which_cond"] == "spatial_affine":
        ...
        self.skip_orig_img_cond_pt0 = cond_module(
            ...
            dim_latent_w=self.cfg["latent_w_dim"], ks=self.cfg["dec_conv_ks"], spade_cond_fn=self.cfg["spade_cond_fn"]
            )
        ...
        self.skip_1_cond = cond_module(
            cfg_txt=f"{self.cfg['cond_cfg_txt']}_hid{n_ch_cond_hidden_1}", in_channels=enc_n_ch["1"], out_channels=enc_n_ch["1"],
            dim_latent_w=self.cfg["latent_w_dim"], ks=self.cfg["skip_conv_ks"], spade_cond_fn=self.cfg["spade_cond_fn"]
            )
        ...
        self.final_cond_0 = cond_module(
            cfg_txt=f"{self.cfg['cond_cfg_txt']}_hid{n_ch_cond_hidden_1}", in_channels=enc_n_ch["1"]+n_ch_decoder_1, out_channels=self.cfg["n_ch_before_output"], 
            dim_latent_w=self.cfg["latent_w_dim"], ks=self.cfg["dec_conv_ks"], spade_cond_fn=self.cfg["spade_cond_fn"]
            )
        # From feature map to the final prediction
        self.conv_last = nn.Conv2d(in_channels=self.cfg["n_ch_before_output"], out_channels=self.cfg["net_output_ch"], kernel_size=1)
    ...
    def forward_composite_task(self, x, task_palette, considered_task_ids):
        ...
        latent_w_vectors = self.compute_latent_w_for_considered_tasks(considered_task_ids=considered_task_ids)
        
        # Make a spatial latent w tensor, by putting task latent codes w at appropriate spatial locations
        orig_shape = x.shape[0:1] + (self.get_latent_w_dim(), ) + x.shape[2:]
        latent_w_spat = self.construct_spatial_latent_w(task_id_map=task_palette, latent_w_vectors=latent_w_vectors, orig_shape=orig_shape, dev=x.device)
        
        # Compute the latent w spatial pyramid
        latent_w_spat_pyramid = self.compute_latent_w_spat_pyramid(latent_w_spat=latent_w_spat)

        # Compute the decoder - final prediction
        out, out_logits = self.compute_decoder(enc_output=enc_output, latent_w_spat_pyramid=latent_w_spat_pyramid)
    ...
    def compute_latent_w_spat_pyramid(self, latent_w_spat):
        ...
        for glob_stride in [2, 4, 8, 16, 32]:
            ...
            if self.cfg["latent_w_spat_interpolation"] == "bilinear_seq":
                # "*_seq" means current pyramid member is interpolated from the previous one
            ...
            elif self.cfg["latent_w_spat_interpolation"] == "bilinear_from_orig":
                # "*_from_orig" means current pyramid member is interpolated from the original size of task code w spatial map
                latent_w_spat_pyr[f"s{glob_stride}"] = F.interpolate(latent_w_spat, size=(orig_shape[2]//glob_stride, orig_shape[3]//glob_stride), mode='bilinear')
            elif self.cfg["latent_w_spat_interpolation"] == "nearest_seq":
                # "*_seq" means current pyramid member is interpolated from the previous one
                latent_w_spat_pyr[f"s{glob_stride}"] = F.interpolate(latent_w_spat_pyr[f"s{int(glob_stride/2)}"], size=(prev_shape[2]//2, prev_shape[3]//2), mode='nearest')
            elif self.cfg["latent_w_spat_interpolation"] == "nearest_from_orig":
                # "*_from_orig" means current pyramid member is interpolated from the original size of task code w spatial map
                latent_w_spat_pyr[f"s{glob_stride}"] = F.interpolate(latent_w_spat, size=(orig_shape[2]//glob_stride, orig_shape[3]//glob_stride), mode='nearest')
            else:
                raise NotImplementedError
    ...
    def _get_n_channels_in_enc_levels(self):
        if self.cfg["backbone_arch"] in ["resnet18", "resnet34"]:
            n_ch = {
                "1": 64,
                "2": 64,
                "3": 128,
        ...
        elif self.cfg["backbone_arch"] in ["resnet50", "resnet101"]:
            n_ch = {
                "1": 64,
                "2": 256,
                "3": 512,
    ...
    def get_decoder_parameters_without_bn(self):
        """
        Get the parameters of the modules that are usually training from scratch
        (Decoder + layers on the encoders skip connections)
        """
        parameters = [
            self.skip_orig_img_cond_pt0.get_all_param_except_bn(),
            self.skip_orig_img_cond_pt1.get_all_param_except_bn(),
            self.skip_1_cond.get_all_param_except_bn(), 
            self.skip_2_cond.get_all_param_except_bn(),
            self.skip_3_cond.get_all_param_except_bn(),
            self.skip_4_cond.get_all_param_except_bn(),
            self.skip_5_cond.get_all_param_except_bn(),

            self.upsample.parameters(),

            self.regular_4_cond.get_all_param_except_bn(),
            self.regular_3_cond.get_all_param_except_bn(),
            self.regular_2_cond.get_all_param_except_bn(),
            self.regular_1_cond.get_all_param_except_bn(),

            self.final_cond_0.get_all_param_except_bn(),
        ]
        return parameters
    
            
    def get_decoder_bn_parameters(self):
        """
        Get the parameters of the modules that are usually training from scratch
        (Decoder + layers on the encoders skip connections)
        """
        parameters = [
            ...
            self.skip_orig_img_cond_pt0.get_bn_parametern(),
            self.skip_orig_img_cond_pt1.get_bn_parametern(),
            self.skip_1_cond.get_bn_parametern(), 
            self.skip_2_cond.get_bn_parametern(),
            self.skip_3_cond.get_bn_parametern(),
            self.skip_4_cond.get_bn_parametern(),
            self.skip_5_cond.get_bn_parametern(),

            self.regular_4_cond.get_bn_parametern(),
            self.regular_3_cond.get_bn_parametern(),
            self.regular_2_cond.get_bn_parametern(),
            self.regular_1_cond.get_bn_parametern(),

            self.final_cond_0.get_bn_parametern(),
            self.conv_last.parameters(),
        ]
        return parameters
